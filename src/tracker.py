    ...
    with tf.Session() as sess:
        ...
        for i in range(1, num_frames):        
            ...
            if run.visualization:
                track = image_[...,::-1].astype(np.uint8)
                bbox = bboxes[i,:]
                x1,y1 = map(int, [bbox[0],bbox[1]])
                x2,y2 = map(int, [x1+bbox[2], y1+bbox[3]])
                cv2.rectangle(track, (x1,y1), (x2,y2), (255,0,0), 2)
...
def track_cam(hp, run, design, final_score_sz, image, templates_z, scores, start_frame):
    global drag, pos_x, pos_y, target_w, target_h, initialized
    # stores tracker's output for evaluation
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            track = image_.copy()[...,::-1].astype(np.uint8)
            x1,y1 = map(int, [bbox[0],bbox[1]])
            x2,y2 = map(int, [x1+bbox[2], y1+bbox[3]])
            cv2.rectangle(track, (x1,y1), (x2,y2), (255,0,0), 2)
            cv2.imshow('track', track)
